...
public class Draw {
    public CategoryChart bar(Map<String, Long> chartMap, Integer lim) throws IOException {
        ...
        CategoryChart chart = new CategoryChartBuilder().width(1024).height(768).title("histogram ").xAxisTitle("countries").yAxisTitle("count").theme(Styler.ChartTheme.GGPlot2).build();
        chart.getStyler().setLegendPosition(Styler.LegendPosition.InsideNE);
        chart.getStyler().setXAxisLabelRotation(45);
        chart.addSeries("count of countries", keys.stream().limit(lim).collect(Collectors.toList()), val.subList(0,lim));
        return  chart;
    }


     public PieChart pi(Map<String, Long> chartMap, Integer lim) throws IOException {
        List<String> keys = chartMap.keySet().stream().collect(Collectors.toList());
        List<Long> val    = chartMap.values().stream().collect(Collectors.toList());
